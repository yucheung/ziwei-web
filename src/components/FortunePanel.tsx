...
export function FortunePanel({
  ...
  if (!astrolabe) {
    return (
      <div className="glass-panel p-8 rounded-2xl border border-slate-800 flex flex-col items-center justify-center text-center space-y-4 text-slate-400 min-h-[360px]">
        <div className="p-4 rounded-full bg-slate-900 border border-slate-800 text-amber-400">
          <TrendingUp className="w-8 h-8 opacity-80" />
        </div>
        <div className="space-y-1 max-w-sm">
          <h3 className="text-base font-semibold text-slate-200">未載入星盤資料</h3>
  ...
  if (!summary) {
    return (
      <div className="glass-panel p-6 rounded-2xl border border-rose-900/50 bg-rose-950/20 text-rose-300 flex items-center gap-3">
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="glass-panel p-6 rounded-2xl border border-slate-800 space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800/80 pb-4">
          ...
          <div className="flex items-center gap-2">
            <span className="px-3 py-1 rounded-lg bg-amber-500/10 border border-amber-500/20 text-amber-300 text-xs font-semibold font-mono">
        ...
        <div className="flex flex-wrap items-center gap-3 pt-1">
          <div className="flex items-center gap-2 bg-slate-900/90 px-3 py-1.5 rounded-xl border border-slate-800">
            ...
            <label htmlFor="target-date-input" className="text-xs text-slate-400 font-medium">
              切換查詢日期：
            </label>
            <input
              ...
              className="bg-transparent text-xs font-mono text-slate-100 focus:outline-none cursor-pointer"
            />
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {/* Decadal Card */}
        <div className="glass-panel p-5 rounded-2xl border border-slate-800 bg-gradient-to-br from-indigo-950/30 to-slate-900/60 space-y-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <div className="w-2 h-2 rounded-full bg-indigo-400 animate-pulse" />
              <h3 className="text-sm font-bold text-indigo-300">當前大限 (十年運勢)</h3>
            </div>
          ...
          <div className="flex items-baseline justify-between pt-1">
            <div className="space-y-0.5">
              <span className="text-xs text-slate-400">大限命宮重疊本命</span>
      ...
      <div className="glass-panel p-6 rounded-2xl border border-slate-800 space-y-4">
        <div className="flex items-center justify-between border-b border-slate-800 pb-3">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-base font-semibold text-slate-200">大限運勢推算表 (10年大限)</h3>
          </div>
          <span className="text-xs text-slate-400">綠色 Highlight 為當前大限所在</span>
        </div>
                  ...
                  >
                    ...
                    <td className="py-3 px-3 font-sans">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-[11px] px-1.5 py-0.5 rounded bg-sky-500/10 text-sky-400 border border-sky-500/20">
                          科:{item.mutagen.ke}
                        </span>
                        <span className="text-[11px] px-1.5 py-0.5 rounded bg-rose-500/10 text-rose-400 border border-rose-500/20">
                          忌:{item.mutagen.ji}
                        </span>
                      </div>
                    </td>
                    <td className="py-3 px-3 text-right font-sans">
                      {item.isCurrent ? (
                        ...
                        <span className="text-[11px] text-slate-500">一</span>
                      )}
                    </td>
                  </tr>
                );
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
          {astrolabe.palaces.map((palace: any, idx: number) => {
            ...
            return (
              ...
              >
                {/* Header */}
                <div className="flex items-center justify-between font-bold border-b border-slate-800/60 pb-1.5">
                  ...
                  {(scopeStars.suiqianStar || scopeStars.jiangqianStar) && (
                    <div className="flex items-center gap-1.5 text-[10px] text-slate-400 pt-0.5">
                      {scopeStars.suiqianStar && (
                        <span className="text-slate-400">歲:{scopeStars.suiqianStar}</span>
                      )}
                      {scopeStars.jiangqianStar && (
                        <span className="text-slate-400">將:{scopeStars.jiangqianStar}</span>
                      )}
                    </div>
                  )}
                </div>
