...
export function FortunePanel({
  ...
  if (!astrolabe) {
    return (
      <div className="glass-panel p-8 rounded-2xl border border-slate-200 dark:border-slate-800 flex flex-col items-center justify-center text-center space-y-4 text-slate-400 dark:text-slate-500 dark:text-slate-400 min-h-[360px]">
  ...
  if (!summary) {
    return (
      <div className="glass-panel p-6 rounded-2xl border border-rose-900/50 bg-rose-100/50 dark:bg-rose-950/20 text-rose-600 dark:text-rose-300 flex items-center gap-3">
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="glass-panel p-6 rounded-2xl border border-slate-200 dark:border-slate-800 space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200/ dark:border-slate-800/80 pb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 dark:text-slate-500 dark:text-slate-400 font-mono">
          ...
          <div className="flex items-center gap-2">
            <span className="px-3 py-1 rounded-lg bg-amber-500/10 border border-amber-500/20 text-amber-600 dark:text-amber-300 text-xs font-semibold font-mono">
        ...
        <div className="flex flex-wrap items-center gap-3 pt-1">
          <div className="flex items-center gap-2 bg-slate-100/ dark:bg-slate-900/90 px-3 py-1.5 rounded-xl border border-slate-200 dark:border-slate-800">
            ...
            <label htmlFor="target-date-input" className="text-xs text-slate-400 dark:text-slate-500 dark:text-slate-400 font-medium">
              切換查詢日期：
            </label>
            <input
            ...
            <button
              ...
              className={`px-2.5 py-1 text-xs rounded-lg border transition-all ${
                targetDate.startsWith(String(currentYear))
                  ...
                  : 'bg-slate-100 dark:bg-slate-900 border-slate-200 dark:border-slate-800 text-slate-400 dark:text-slate-500 dark:text-slate-400 hover:text-slate-200'
              }`}
            >
              今年 ({currentYear})
            </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {/* Decadal Card */}
        <div className="glass-panel p-5 rounded-2xl border border-slate-200 dark:border-slate-800 bg-gradient-to-br from-indigo-100/50 dark:from-indigo-100/ dark:from-indigo-950/30 to-slate-50/60 dark:to-slate-900/60 space-y-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <div className="w-2 h-2 rounded-full bg-indigo-400 animate-pulse" />
              <h3 className="text-sm font-bold text-indigo-600 dark:text-indigo-300">當前大限 (十年運勢)</h3>
            </div>
            <span className="text-xs font-mono px-2 py-0.5 rounded bg-indigo-500/20 text-indigo-600 dark:text-indigo-300 border border-indigo-500/30">
          ...
          <div className="flex items-baseline justify-between pt-1">
            <div className="space-y-0.5">
              <span className="text-xs text-slate-400 dark:text-slate-500 dark:text-slate-400">大限命宮重疊本命</span>
          ...
          <div className="pt-2 border-t border-slate-200/ dark:border-slate-800/80">
            <span className="block text-[11px] font-medium text-slate-400 dark:text-slate-500 dark:text-slate-400 mb-1.5">
              大限天干 [{summary.decadal.stemBranch.charAt(0)}] 四化引動：
            </span>
            <div className="grid grid-cols-4 gap-1.5 text-center text-xs font-medium">
              <div className="px-2 py-1 rounded bg-emerald-500/10 border border-emerald-500/30 text-emerald-500 dark:text-emerald-400">
                <span className="block text-[10px] text-emerald-500/80">化祿</span>
                {summary.decadal.mutagen.lu}
              </div>
              <div className="px-2 py-1 rounded bg-purple-500/10 border border-purple-500/30 text-purple-500 dark:text-purple-400">
                <span className="block text-[10px] text-purple-500/80">化權</span>
                {summary.decadal.mutagen.quan}
              </div>
              <div className="px-2 py-1 rounded bg-sky-500/10 border border-sky-500/30 text-sky-500 dark:text-sky-400">
                <span className="block text-[10px] text-sky-500/80">化科</span>
                {summary.decadal.mutagen.ke}
              </div>
              <div className="px-2 py-1 rounded bg-rose-500/10 border border-rose-500/30 text-rose-500 dark:text-rose-400">
        ...
        <div className="glass-panel p-5 rounded-2xl border border-slate-200 dark:border-slate-800 bg-gradient-to-br from-amber-100/50 dark:from-amber-950/20 to-slate-50/60 dark:to-slate-900/60 space-y-3">
          {/* Level Tab Buttons */}
              ...
              <button
                ...
                className={`flex-1 px-2 py-1.5 text-xs rounded-md font-medium transition-all ${
                  fortuneLevel === level
                    ...
                    : 'text-slate-400 dark:text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:text-slate-200 border border-transparent'
                }`}
              >
          ...
          <div className="flex items-baseline justify-between pt-1">
            <div className="space-y-0.5">
              <span className="text-xs text-slate-400 dark:text-slate-500 dark:text-slate-400">{FORTUNE_LEVEL_LABELS[fortuneLevel].zh}命宮重疊本命</span>
              <div className="text-lg font-bold text-slate-900 dark:text-slate-100 flex items-center gap-1.5">
      ...
      <div className="glass-panel p-6 rounded-2xl border border-slate-200 dark:border-slate-800 space-y-4">
        <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-3">
          ...
          <span className="text-xs text-slate-400 dark:text-slate-500 dark:text-slate-400">綠色 Highlight 為當前大限所在</span>
        </div>

        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs text-slate-700 dark:text-slate-300">
            <thead className="bg-slate-100/ dark:bg-slate-900/80 text-slate-400 dark:text-slate-500 dark:text-slate-400 font-medium border-b border-slate-200 dark:border-slate-800">
              <tr>
                  ...
                  <tr
                    ...
                    className={`transition-colors cursor-pointer ${
                      item.isCurrent
                        ...
                        : 'hover:bg-slate-100/ dark:bg-slate-900/60'
                    }`}
                  >
                    ...
                    <td className="py-3 px-3 text-slate-400 dark:text-slate-500 dark:text-slate-400 font-bold">
                      {item.stemBranch}
                    </td>
                    ...
                    <td className="py-3 px-3 font-sans">
                      <div className="flex items-center gap-1.5">
                        <span className="text-[11px] px-1.5 py-0.5 rounded bg-emerald-500/10 text-emerald-500 dark:text-emerald-400 border border-emerald-500/20">
                          祿:{item.mutagen.lu}
                        </span>
                        <span className="text-[11px] px-1.5 py-0.5 rounded bg-purple-500/10 text-purple-500 dark:text-purple-400 border border-purple-500/20">
                          權:{item.mutagen.quan}
                        </span>
                        <span className="text-[11px] px-1.5 py-0.5 rounded bg-sky-500/10 text-sky-500 dark:text-sky-400 border border-sky-500/20">
                          科:{item.mutagen.ke}
                        </span>
                        <span className="text-[11px] px-1.5 py-0.5 rounded bg-rose-500/10 text-rose-500 dark:text-rose-400 border border-rose-500/20">
                          忌:{item.mutagen.ji}
                        </span>
                      </div>
                    </td>
                    <td className="py-3 px-3 text-right font-sans">
                      {item.isCurrent ? (
                        <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-amber-500 text-slate-950 shadow-sm shadow-amber-500/30">
      ...
      <div className="glass-panel p-6 rounded-2xl border border-slate-200 dark:border-slate-800 space-y-4">
        <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-3">
          ...
          <span className="text-xs text-slate-400 dark:text-slate-500 dark:text-slate-400">大限流曜 / {FORTUNE_LEVEL_LABELS[fortuneLevel].zh}流曜 / 歲前將前神</span>
        </div>
            ...
            return (
              <div
                ...
                className={`p-3 rounded-xl border text-xs space-y-2 transition-all ${
                  isLevelLife
                    ? 'bg-amber-100/50 dark:bg-amber-950/20 border-amber-500/40 shadow-sm shadow-amber-500/10'
                    : isDecadalLife
                    ? 'bg-indigo-100/50 dark:bg-indigo-950/20 border-indigo-500/40'
                    : 'bg-slate-100/ dark:bg-slate-900/60 border-slate-200 dark:border-slate-800 hover:border-slate-700'
                }`}
              >
                {/* Header */}
                <div className="flex items-center justify-between font-bold border-b border-slate-200/ dark:border-slate-800/60 pb-1.5">
                  ...
                  <div className="flex items-center gap-1 text-[10px]">
                    {isDecadalLife && (
                      <span className="px-1.5 py-0.5 rounded bg-indigo-500/20 text-indigo-600 dark:text-indigo-300">
                        限命
                      </span>
                    )}
                    {isLevelLife && (
                      <span className="px-1.5 py-0.5 rounded bg-amber-500/20 text-amber-600 dark:text-amber-300">
                ...
                {/* Overlap Roles */}
                <div className="text-[11px] text-slate-400 dark:text-slate-500 dark:text-slate-400 space-x-2 font-mono">
                ...
                <div className="space-y-1 pt-1">
                  ...
                  {scopeStars.decadalStars.length > 0 && (
                    <div className="flex flex-wrap gap-1">
                      {scopeStars.decadalStars.map((star: string, sIdx: number) => (
                        <span
                          key={sIdx}
                          className="px-1.5 py-0.5 text-[10px] rounded bg-indigo-500/10 text-indigo-600 dark:text-indigo-300 border border-indigo-500/20"
                        >
                          {star}
                        </span>
                      ))}
                  ...
                  {(scopeStars.suiqianStar || scopeStars.jiangqianStar) && (
                    <div className="flex items-center gap-1.5 text-[10px] text-slate-400 dark:text-slate-500 dark:text-slate-400 pt-0.5">
                      {scopeStars.suiqianStar && (
                        <span className="text-slate-400 dark:text-slate-500 dark:text-slate-400">歲:{scopeStars.suiqianStar}</span>
                      )}
                      {scopeStars.jiangqianStar && (
                        <span className="text-slate-400 dark:text-slate-500 dark:text-slate-400">將:{scopeStars.jiangqianStar}</span>
                      )}
                    </div>
                  )}
                </div>
