...
export function Settings({ config, setConfig, astroType, setAstroType }: SettingsProps) {
  ...
  return (
    <div className="mt-4 pt-4 border-t border-slate-200 dark:border-slate-800">
      ...
      {/* 流派切換 */}
      <div className="mb-3">
        <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">{t('settings.school')}</label>
      ...
      {/* 三盤切換 */}
      <div className="mb-3">
        <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">{t('settings.astroType')}</label>
      ...
      {/* 晚子時 / 年界 */}
      <div className="mb-3">
        <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">{t('settings.lateZi')}</label>
        <select
          value={config.dayDivide ?? 'current'}
          onChange={(e) => setConfig((c) => ({ ...c, dayDivide: e.target.value as 'current' | 'forward' }))}
          className="w-full px-2 py-1.5 rounded-lg bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700/80 text-slate-900 dark:text-slate-100 text-xs focus:outline-none focus:ring-2 focus:ring-amber-500/50"
        >
      ...
      <div>
        <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">{t('settings.yearBoundary')}</label>
        <select
