...
> 建立日期：2026-08-06
> 最後更新：2026-08-06（D1-D5 決策確認 + V1-V4 修正）
> 狀態：**已確認（V1-V4 待技術驗證）**
...
| **LLM 管線** | prompts.ts 參數化（system prompt + 5 類 type prompt + locale）| ✅ 完整（但 prompt 需修正）|
...
- **立即修正** prompts.ts：「兼通三合派與飛星派」→「三合派」（飛星尚未實作，不應宣稱兼通，否則 LLM 可能自行混派）
...
**決策**：保留四柱顯示 + 博士十二神，明確標示「資料顯示，尚未提供完整八字推論」。暫停「簡單 LLM 八字解讀」——看似成本最低，實際最容易讓使用者誤以為系統已處理月令、旺衰、十神、調候和喜用神。
...
| prompts.ts 修正 | 移除「兼通三合派與飛星派」→ 標示「三合派」 | 小 | 無 |
...
1. **立即修正** prompts.ts：「兼通三合派與飛星派」→「三合派」
2. **拆 Phase 0 為具體 Issues**（排盤面板、JSON 匯出、回歸測試框架、輸入檢視）
3. **開始 Phase 0 實施**
4. **同步進行 V1-V4 技術驗證**（用現有程式碼實測 iztro API）
